...
(function (global) {
  const DOC_GROUPS = [
    {
      ...
      items: [
        ...
        { file: "页面-模块映射表.md", label: "页面 · 模块映射表" },
  ...
  const VIEW_PAGE_MEANING = {
    ...
    channelSales: "运营商维护签约渠道、渠道订单、链接结算（链接类佣金）与已售权益；平台侧另有只读监管。",
    ...
    dayPool_pools: "本页是「池库存台账」：按签约运营商查看总购、赠送、预占、已消耗与可用余额。余额不足 = 可用 < 在职×10（与顶栏/短信同一条）；点开单池看详情/续费。给人配天数请去「额度分配」，本页不配额度。",
  ...
  const VIEW_PAGE_SCOPE = {
    ...
    dayPool_pools: {
      does: "看各池库存与可用余额；余额不足=可用<在职×10；点开单池看详情与续费入口。",
  ...
  const VIEW_PAGE_RULES = {
    ...
    channelLinks: [
      ...
      { summary: "首次点击有效链接起二十四小时内首笔购授权套餐（无在服套餐、无历史个人购卡）享专享价；续费与再次购卡走正式零售价。", file: "渠道结算模式规则.md", heading: "24 小时归因窗口" },
